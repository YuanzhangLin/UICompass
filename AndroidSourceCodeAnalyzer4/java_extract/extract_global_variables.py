...
def get_global_variables_info(tree, source_code, output_json_path):
    ...
    def find_global_variables(node):
        variables = []
        if node.type == 'field_declaration':  # Tree-sitter 定义字段为 field_declaration
            # 获取字段修饰符、类型和名称
            modifiers = []
            var_type = None
            var_name = None
            ...
            var_type = source_code[node.child_by_field_name('type').start_byte:node.child_by_field_name('type').end_byte].decode('utf-8').strip()
